...
describe("Debt Token Contract Wrapper (Unit)", () => {
    ...
    describe("#deployed()", () => {
        describe("local artifacts are nonexistent", () => {
            ...
            test("throws ARTIFACTS_NOT_READABLE error", async () => {
                await expect(DebtTokenContract.deployed(web3, TX_DEFAULTS)).rejects
                    .toThrowError(CONTRACT_WRAPPER_ERRORS.ARTIFACTS_NOT_READABLE("DebtToken"));
            });
        });
        ...
        describe("no contract address associated w/ current network id", () => {
            ...
            test("throws CONTRACT_NOT_FOUND_ON_NETWORK error", async () => {
                ...
                await expect(DebtTokenContract.deployed(web3, TX_DEFAULTS)).rejects
                    .toThrowError(CONTRACT_WRAPPER_ERRORS.CONTRACT_NOT_FOUND_ON_NETWORK("DebtToken", networkId));
            });
        });
    ...
    describe("#at()", () => {
        describe("local artifacts are nonexistent", () => {
            ...
            test("throws ARTIFACTS_NOT_READABLE error", async () => {
                await expect(DebtTokenContract.at(debtTokenContractAddress, web3, TX_DEFAULTS)).rejects
                    .toThrowError(CONTRACT_WRAPPER_ERRORS.ARTIFACTS_NOT_READABLE("DebtToken"));
            });
        });
        ...
        describe("contract address does not point to contract", () => {
            ...
            test("throws CONTRACT_NOT_FOUND_ON_NETWORK error", async () => {
                await expect(DebtTokenContract.at(ACCOUNTS[0].address, web3, TX_DEFAULTS)).rejects
                    .toThrowError(CONTRACT_WRAPPER_ERRORS.CONTRACT_NOT_FOUND_ON_NETWORK("DebtToken", networkId));
            });
        });
        ...
        describe("local artifacts readable and contract address associated w/ network id is valid", () => {
            ...
            test("returns new DebtKernelWrapper w/ current address correctly set", async () => {
                const contractWrapper = await DebtTokenContract.at(debtTokenContractAddress, web3, TX_DEFAULTS);
